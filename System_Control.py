        ...
        for j in range(len(WBANList)):
            ...
            if finishBuffer[i].timeslice == timeReal and finishBuffer[i].numOfWBAN == WBANList[j].number:
                ...
                if finishBuffer[i].ifOffload == 0:
                    Profit_WBAN[j] += finishBuffer[i].value - alpha * finishBuffer[i].timeLocal - Beta * finishBuffer[
                        i].energyLocal
                # 如果该任务卸载执行
                elif finishBuffer[i].ifOffload == 1:
                    Profit_WBAN[j] += finishBuffer[i].value - alpha * finishBuffer[i].timeTransmit - Beta * \
                                                                                                     finishBuffer[
                                                                                                         i].energyTransmit - \
                                      finishBuffer[i].payForMEC
                    Profit_MEC += finishBuffer[i].payForMEC - alpha * finishBuffer[i].timeMEC - Beta * finishBuffer[
                        i].energyMEC
